...
/**
 * Classe che gestisce la creazione, in formato pdf, ed il salvataggio del badge
 * relativo ai soci del circolo
 * 
 * @author simoneonori
 * @author eliapacioni
...
public class PdfController {
	...
	private static Font bigFont = new Font(Font.FontFamily.TIMES_ROMAN, 10, Font.ITALIC);
	private static Font smallBold = new Font(Font.FontFamily.TIMES_ROMAN, 8, Font.ITALIC);
	private static Font formalFont = new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.ITALIC);

	public void gestioneBadge() {
	...
	private void controlloEventiBadge() {
		 ...
		 * cognome, cf, tipologia e città) del socio puntato nella combobox
		 */
		...
		/**
		 * Gestione evento per il funzionamento del tasto "Crea PDF" che porterà
		...
		viewBadge.getBtnCreaPdfBadge().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try {
					...
					document.close();
					/**
					 * Utile per informare dove è stata rilevata l'eccezione
	...
	private static String showSaveFileDialogBadge(Document document) {
	...
	void aggiungiPrefazioneBadge(Document document) throws DocumentException {
		...
		prefazioneBadge.add(new Paragraph("Nome: " + soci.get(viewBadge.getComboBoxSocio().getSelectedIndex()).getNome(),
				smallBold));
		...
		prefazioneBadge.add(new Paragraph("Città: " + soci.get(viewBadge.getComboBoxSocio().getSelectedIndex()).getCitta(),
				smallBold));
	...
	/**
	 * Metodo per visualizzare i soci del circolo che compiono gli anni nell'immediato
	 * visualizzandone il nome, cognome e cf
	 */
	...
	/**
	 * Metodo per gestire gli eventi relativi alla creazione del biglietto di auguri
	 */
	...
	private void controlloEventiBiglietto() {
		...
		/**
		 * Metodo per visualizzare a schermo le informazioni principali(nome,
		 * cognome, cf, tipologia, città e indirizzo) del socio puntato nella combobox
		 */
		...
		/**
		 * Gestione evento per il funzionamento del tasto "Crea PDF Biglietto" che porterà
		 * alla creazione e al salvataggio del biglietto in formato ID3 del socio
		 * puntato nella combobox
		 */
	...
	private static String showSaveFileDialogBiglietto(Document document) {
		JFileChooser fileChooser = new JFileChooser();
