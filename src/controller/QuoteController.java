...
public class QuoteController {
		...
		viewInsert.getCmbbxTipologia().addItemListener(new ItemListener(){
			...
			public void itemStateChanged(ItemEvent e){
				if(viewInsert.getCmbbxTipologia().getSelectedIndex() != -1){
					quotapre = model.getQuotaPrecendente(viewInsert.getCmbbxTipologia().getItemAt(viewInsert.getCmbbxTipologia().getSelectedIndex()));
					if (quotapre != null){
						if(quotapre.getDataF() != null){
							viewInsert.getTxtFieldDataIPre().setText(quotapre.getDataI().toString());
							viewInsert.getTxtFieldDataFPre().setText(quotapre.getDataF().toString());
							viewInsert.getTxtFieldValorePre().setText(String.valueOf(quotapre.getValore()));
						} else {
							viewInsert.getTxtFieldDataIPre().setText(quotapre.getDataI().toString());
							viewInsert.getTxtFieldDataFPre().setText("");
							viewInsert.getTxtFieldValorePre().setText(String.valueOf(quotapre.getValore()));
						}
					}	
				}
		...
		viewInsert.getBtnInserisci().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(validatorInsertForm()){
					boolean esito = false;
					Quota quota = null;
					if(!viewInsert.getTxtFieldDataF().getText().equals("")){
						quota = new Quota(Float.valueOf(viewInsert.getTxtFieldValore().getText()).floatValue(),
								...
								Date.valueOf(viewInsert.getTxtFieldDataI().getText()),
								Date.valueOf(viewInsert.getTxtFieldDataF().getText()));
						...
						quota = new Quota(Float.valueOf(viewInsert.getTxtFieldValore().getText()).floatValue(),
								...
								null);
						if(quotapre != null){
							esito = (model.insertQuota(quota) && model.insertDataFine(quota, quotapre));
						} else {
							esito = (model.insertQuota(quota));
						}
					}
		...
		viewManagement.getBtnElimina().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (JOptionPane.showConfirmDialog(null, "Eliminare la quota?") == 0
					...
					boolean esito = model
							.deleteQuota(quotes.get(viewManagement.getTable().getSelectedRow()).getId());
	...
	private void fillTableQuote() {
		String[] nameColumns = { "Id", "Data inizio", "Data fine", "Valore", "Tipologia" };
		...
		for (int j = 0; j < quotes.size(); j++) {
			...
			dati.setValueAt(quotes.get(j).getDataF(), j, 2);
			dati.setValueAt(quotes.get(j).getValore(), j, 3);
			dati.setValueAt(quotes.get(j).getTipologia(), j, 4);
		}
	...
	private boolean validatorInsertForm() {
		...
		if (!viewInsert.getTxtFieldDataF().getText().equals("")) {
			if (!Validator.ValidaDataFine(viewInsert.getTxtFieldDataF().getText())) {
				viewInsert.getTxtFieldDataF().setBackground(Color.red);
				validazione = false;
			} else {
				if (viewInsert.getTxtFieldDataF().getBackground() == Color.red)
					viewInsert.getTxtFieldDataF().setBackground(Color.white);
			}
		}
		...
		if (!viewInsert.getTxtFieldDataI().getText().equals("") && !viewInsert.getTxtFieldDataIPre().getText().equals("")){
		...
		if (!viewInsert.getTxtFieldDataI().getText().equals("") && !viewInsert.getTxtFieldDataFPre().getText().equals("")){
			if (!Validator.ValidaDataMaggiore(Date.valueOf(viewInsert.getTxtFieldDataFPre().getText()), 
						Date.valueOf(viewInsert.getTxtFieldDataI().getText()))){
				viewInsert.getTxtFieldDataI().setBackground(Color.red);
				validazione = false;
			} else {
	...
	private void azzeraInsertForm(){
		...
		viewInsert.getTxtFieldDataF().setText("");
